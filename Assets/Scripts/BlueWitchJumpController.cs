...
public class BlueWitchJumpController : MonoBehaviour
{
    [Header ("References")]
    ...
    void Update()
    {
        ...
        // where the character will jump
         ...
         charging = Input.GetMouseButton(0) && canJump;
          if (onGround){
              canJump = true;
          }
        ...
        if (Input.GetMouseButtonUp(0) && canJump){
            ...
            StartCoroutine(removeTrajectory()); // removes trajectory points after 2.5 seconds -- for any not collided with
